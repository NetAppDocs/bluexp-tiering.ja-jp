...
ライセンス:: データをStorageGRID に階層化する場合、Cloud Tieringライセンスは、BlueXPアカウントには必要ありません。また、ONTAP クラスタにFabricPool ライセンスが必要です。
クラスタネットワークの要件::
+
--
...
+
** ポート443を介したCloud TieringサービスへのHTTPS接続 (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-on-prem.html#endpoints-contacted-for-day-to-day-operations["エンドポイントのリストを参照してください"^])
